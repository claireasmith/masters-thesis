...
PGLS model showed that size and sex system both have a significant effect on pollen production at alpha=0.05. 
...
qqline(modA_PGLS$phyres) #looks okay? deviates from the line at the extremes but is otherwise pretty close
...
```{r inspect distributions mixed mod}
# pollen size: 
ggplot(sizenum_filt) + 
  geom_histogram(aes(x=Avg_diam)) + 
  theme_cs()
...
```{r}
mod.fit <- sizenum_filt %>% as.data.frame()
fit.mod <- fitted(prodsize_mixmod)
mod.fit <- mutate(mod.fit, fit.mod)
emms <- emmip(prodsize_mixmod, ~ Avg_diam | Sex_sys, cov.reduce=range,
              type="response", plotit=T)
emmip(prodsize_mixmod, ~ Avg_diam, cov.reduce=range,
              type="response", plotit=T)
```
...
ggplot() + 
  geom_point(data = mod.fit, aes(x=Avg_diam, y=fit.mod)) 
# doesn't work from here on out
# + 
#   geom_line(data= emms, aes(xvar, yvar), linewidth=0.7) + 
#   # geom_smooth(data=mod.fit, aes(x=Log_stigma_length, y=fit.AMARmod),
#   #             method="lm",
...
sizenum_filt # I already filtered this data for the mixed model analysis for pollen production, there should be no species with fewer
# than 10 members
# sample size per species
print(sizenum_filt %>% group_by(Sex_sys, Species) %>% summarize(n=n()), n=Inf) 
