...
unique(stigph$Species) # 19 species!
#  [1] Dichanthelium implicatum Plantago lanceolata      Phleum pratense          Setaria viridis         
#  [5] Thalictrum dioicum       Plantago major           Elymus repens            Bromus inermis          
#  [9] Elymus innovatus         Festuca campestris       Chenopodium album        Carex communis          
# [13] Carex hirtifolia         Carex pedunculata        Carex plantaginea        Carex stipata           
# [17] Rumex crispus            Scirpus microcarpus      Schizacne purpurascens 
# View(stigph)
...
# 13 Plantago lanceolata         42
# 14 Plantago major              15
# 15 Rumex crispus              108
# 16 Schizacne purpurascens     154
# 17 Scirpus microcarpus        137
# 18 Setaria viridis             45
# 19 Thalictrum dioicum          59
...
BRINmod <- lmer(Log_flw_pollen ~ Infl_max + Date + (1|Ind_ID), data=BRIN)
summary(BRINmod)
anova(BRINmod)
...
anova(CACOmod)
# no sig quadratic term
CACOmod <- lmer(Log_flw_pollen ~ Infl_max + Date + (1|Ind_ID), data=CACO)
summary(CACOmod)
anova(CACOmod)
...
anova(CAHImod)
# no sig quadratic term
CAHImod <- lmer(Log_flw_pollen ~ Infl_max + Date + (1|Ind_ID), data=CAHI)
summary(CAHImod)
anova(CAHImod)
...
anova(CASTmod)
# no sig quadratic term
CASTmod <- lmer(Log_flw_pollen ~ Infl_max + Date + (1|Ind_ID), data=CAST)
# boundary (singular) fit
summary(CASTmod)
anova(CASTmod)
# Significant effect of height and date on pollen loads in Carex stipata. 
```
...
hist(PLMA$Infl_max) # looks ok
```

```{r flower height vs pollen load Plantago major}
# Look at plot of Log_flw_pollen and Flw_height 
# Looks like no big differences between sites in flower height or pollen load
ggplot(data=PLMA, aes(x=Infl_max, y=Log_flw_pollen, color=Date, shape=Site)) + 
  geom_point(size=3) + 
  scale_x_continuous(name="Max height") + 
  scale_y_continuous(name="Log stigmatic pollen load") + 
  theme_cs()
...
#model for height
PLMAmod <- lmer(Log_flw_pollen ~ poly(Infl_max, 2) + (1|Ind_ID), data=PLMA)
summary(PLMAmod)
anova(PLMAmod)
# no sig quadratic term
PLMAmod <- lmer(Log_flw_pollen ~ Infl_max + (1|Ind_ID), data=PLMA)
summary(PLMAmod)
anova(PLMAmod)
# No significant effect of height on pollen loads in Plantago major. 
```
...
# summaryTHDI) 
# Taking only 17 May data (n=29) because I think the earlier dates were too early - not representative of actual pollen capture
...
#model for height
THDImod <- lmer(Log_flw_pollen ~ poly(Infl_max, 2)  + (1|Ind_ID), data=THDI)
summary(THDImod)
anova(THDImod)
# no sig quadratic term
THDImod <- lmer(Log_flw_pollen ~ Infl_max + (1|Ind_ID), data=THDI)
summary(THDImod)
anova(THDImod)
