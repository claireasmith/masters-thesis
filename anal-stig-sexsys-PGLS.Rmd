...
Since lambda was not significantly different from 0 or 1, I'll also do a linear mixed effects model on pollen receipt by sex system with species as random effect.
...
stig_mixmod <- lmer(Log_avg_poll ~ Sex_sys + collector + (1|Species/Plant), data = stig_filt)
summary(stig_mixmod)
anova(stig_mixmod)
```
...
ggplot(data=em_stig, aes(x=Sex_sys, y=emmean)) + 
  ...
  geom_text(aes(x=1, y=log(8)), label="A") + 
  geom_text(aes(x=2, y=log(17)), label="A") + 
  geom_text(aes(x=3, y=log(44)), label="B") + 
