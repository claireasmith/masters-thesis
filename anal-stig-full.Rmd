...
```{r AMAR model selection}
options(na.action = "na.fail") 
# model selection:
AMARmod_dredge <- MuMIn::dredge(AMARmod)
# looks like the top models are log_flw_pollen ~ inv_avg_dist and log_flw_pollen ~ inv_avg_dist + stig_length
# both have AICc values within 2 of each other
head(AMARmod_dredge)
summary(model.avg(AMARmod_dredge, subset = delta <= 2))
AMARmodsum <- summary(model.avg(AMARmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
AMARmodsum <- AMARmodsum %>% mutate(Species="Ambrosia artemisiifolia")
```
...
```{r CACO model selection}
# model selection:
CACOmod_dredge <- MuMIn::dredge(CACOmod)
head(CACOmod_dredge)
# top model has date, inv avg dist and stig length
# summary(model.avg(CACOmod_dredge, subset = delta <= 2))
# CACOmodsum <- summary(model.avg(CACOmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
CACOmodsum <- summary(get.models(CACOmod_dredge, subset = 1)[[1]])
```
...
```{r CAHI model selection}
# model selection:
CAHImod_dredge <- MuMIn::dredge(CAHImod)
head(CAHImod_dredge)
# top model has inv avg dist and stig length
# summary(model.avg(CACOmod_dredge, subset = delta <= 2))
# CACOmodsum <- summary(model.avg(CACOmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
CAHImodsum <- summary(get.models(CAHImod_dredge, subset = 1)[[1]])
```
...
```{r CAPE model selection}
# model selection:
CAPEmod_dredge <- MuMIn::dredge(CAPEmod)
head(CAPEmod_dredge)
# top model has date and inv avg dist
# summary(model.avg(CACOmod_dredge, subset = delta <= 2))
# CACOmodsum <- summary(model.avg(CACOmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
CAPEmodsum <- summary(get.models(CAPEmod_dredge, subset = 1)[[1]])
```
...
```{r CAPL model selection}
# model selection:
CAPLmod_dredge <- MuMIn::dredge(CAPLmod)
head(CAPLmod_dredge)
# top model has date and inv avg dist
# summary(model.avg(CACOmod_dredge, subset = delta <= 2))
# CACOmodsum <- summary(model.avg(CACOmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
CAPLmodsum <- summary(get.models(CAPLmod_dredge, subset = 1)[[1]])
```
...
```{r CAST model selection}
# model selection:
CASTmod_dredge <- MuMIn::dredge(CASTmod)
head(CASTmod_dredge)
# top model has date and stig legnth - but at least one model failed to converge
# summary(model.avg(CACOmod_dredge, subset = delta <= 2))
# CACOmodsum <- summary(model.avg(CACOmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
CASTmodsum <- summary(get.models(CASTmod_dredge, subset = 1)[[1]])
```
...
```{r CHAL model selection}
# model selection:
CHALmod_dredge <- MuMIn::dredge(CHALmod)
head(CHALmod_dredge)
# top model has date and stig legnth - but at least one model failed to converge
# summary(model.avg(CACOmod_dredge, subset = delta <= 2))
# CACOmodsum <- summary(model.avg(CACOmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
CHALmodsum <- summary(get.models(CHALmod_dredge, subset = 1)[[1]])
```
...
```{r PLLA model selection}
# model selection:
PLLAmod_dredge <- MuMIn::dredge(PLLAmod)
head(PLLAmod_dredge)
# top model has date and stig legnth - but at least one model failed to converge
# summary(model.avg(CACOmod_dredge, subset = delta <= 2))
# CACOmodsum <- summary(model.avg(CACOmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
PLLAmodsum <- summary(get.models(PLLAmod_dredge, subset = 1)[[1]])
```
...
```{r RUAC model selection}
# model selection:
RUACmod_dredge <- MuMIn::dredge(RUACmod)
head(RUACmod_dredge)
# top model has date, inv avg dist, and stig length
# summary(model.avg(CACOmod_dredge, subset = delta <= 2))
# CACOmodsum <- summary(model.avg(CACOmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
RUACmodsum <- summary(get.models(RUACmod_dredge, subset = 1)[[1]])
```
...
```{r RUCR model selection}
# model selection:
RUCRmod_dredge <- MuMIn::dredge(RUCRmod)
head(RUCRmod_dredge)
# top model has inv avg dist, and stig length
# summary(model.avg(CACOmod_dredge, subset = delta <= 2))
# CACOmodsum <- summary(model.avg(CACOmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
RUCRmodsum <- summary(get.models(RUCRmod_dredge, subset = 1)[[1]])
```
...
```{r SCPU model selection}
# model selection:
SCPUmod_dredge <- MuMIn::dredge(SCPUmod)
head(SCPUmod_dredge)
# top model has inv avg dist, and stig length
# summary(model.avg(CACOmod_dredge, subset = delta <= 2))
# CACOmodsum <- summary(model.avg(CACOmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
SCPUmodsum <- summary(get.models(SCPUmod_dredge, subset = 1)[[1]])
```
...
```{r SCMI model selection}
# model selection:
SCMImod_dredge <- MuMIn::dredge(SCMImod)
head(SCMImod_dredge)
# top model has date and stig length
# summary(model.avg(CACOmod_dredge, subset = delta <= 2))
# CACOmodsum <- summary(model.avg(CACOmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
SCMImodsum <- summary(get.models(SCMImod_dredge, subset = 1)[[1]])
```
...
```{r THDI model selection}
# model selection:
THDImod_dredge <- MuMIn::dredge(THDImod)
head(THDImod_dredge)
# top model has date, inv avg dist and stig length
# summary(model.avg(CACOmod_dredge, subset = delta <= 2))
# CACOmodsum <- summary(model.avg(CACOmod_dredge, subset = delta <= 2))$coefmat.full %>% as.data.frame() 
THDImodsum <- summary(get.models(THDImod_dredge, subset = 1)[[1]])
```
